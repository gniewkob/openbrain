...
from .memory_reads import get_memory_domain_status_counts, get_memory_status_counts
from .telemetry import set_gauge_metric


def build_memory_gauges(
    status_counts: dict[str, int], domain_status_counts: dict[str, dict[str, int]]
) -> dict[str, float]:
...
async def refresh_memory_gauges(session: AsyncSession) -> dict[str, float]:
    ...
    gauges = build_memory_gauges(status_counts, domain_status_counts)
