...
# Security functions live in src.security.policy — patch there, not on main,
# so the patches actually intercept the calls made inside the security module.
...
class AccessControlTests(unittest.IsolatedAsyncioTestCase):
    async def test_create_memory_forces_owner_to_subject_for_scoped_user(self) -> None:
        ...
        with patch.object(main, "store_memory", new=AsyncMock(return_value=_memory_out(owner="user-1"))) as store_memory, patch(
    ...
    async def test_create_memory_forces_tenant_to_claim_for_scoped_tenant_user(self) -> None:
        ...
        with patch.object(main, "store_memory", new=AsyncMock(return_value=_memory_out(tenant_id="tenant-a"))) as store_memory, patch(
            f"{_POLICY}.PUBLIC_MODE", True
        ), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(f"{_POLICY}.get_tenant_id", return_value="tenant-a"), patch(
            f"{_POLICY}._effective_domain_scope", return_value={"build", "corporate", "personal"}
        ):
    ...
    async def test_search_applies_tenant_scope_when_claim_present(self) -> None:
        ...
        with patch.object(main, "search_memories", new=AsyncMock(return_value=[])) as search_memories, patch(
            f"{_POLICY}.PUBLIC_MODE", True
        ), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(f"{_POLICY}.get_tenant_id", return_value="tenant-a"):
    ...
    async def test_create_memory_blocks_write_to_disallowed_domain(self) -> None:
        ...
        with patch(f"{_POLICY}.PUBLIC_MODE", True), patch(f"{_POLICY}.get_domain_scope", return_value={"build"}):
            with self.assertRaises(HTTPException) as ctx:
                await main.create_memory(data=data, session=object(), _user={"sub": "user-1"})
        self.assertEqual(ctx.exception.status_code, 403)

    async def test_read_memories_applies_owner_scope_for_scoped_user(self) -> None:
        with patch.object(main, "list_memories", new=AsyncMock(return_value=[])) as list_memories, patch(
            f"{_POLICY}.PUBLIC_MODE", True
        ), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(f"{_POLICY}.get_subject", return_value="user-1"):
    ...
    async def test_read_memories_blocks_requested_domain_outside_policy(self) -> None:
        with patch(f"{_POLICY}.PUBLIC_MODE", True), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(
            f"{_POLICY}.get_subject", return_value="user-1"
        ), patch(f"{_POLICY}._effective_domain_scope", return_value={"build"}):
            with self.assertRaises(HTTPException) as ctx:
    ...
    async def test_search_applies_owner_scope_for_scoped_user(self) -> None:
        ...
        with patch.object(main, "search_memories", new=AsyncMock(return_value=[])) as search_memories, patch(
            f"{_POLICY}.PUBLIC_MODE", True
        ), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(f"{_POLICY}.get_subject", return_value="user-1"), patch(f"{_POLICY}.get_tenant_id", return_value=""):
    ...
    async def test_update_blocks_access_to_other_tenant_for_scoped_user(self) -> None:
        with patch.object(main, "get_memory", new=AsyncMock(return_value=_memory_out(owner="user-1", tenant_id="tenant-b"))), patch(
            f"{_POLICY}.PUBLIC_MODE", True
        ), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(f"{_POLICY}.get_tenant_id", return_value="tenant-a"), patch(
            f"{_POLICY}._effective_domain_scope", return_value={"build", "corporate", "personal"}
        ):
    ...
    async def test_update_blocks_access_to_other_owner_for_scoped_user(self) -> None:
        with patch.object(main, "get_memory", new=AsyncMock(return_value=_memory_out(owner="other-user"))), patch(
            f"{_POLICY}.PUBLIC_MODE", True
        ), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(f"{_POLICY}.get_subject", return_value="user-1"), patch(
            f"{_POLICY}._effective_domain_scope", return_value={"build", "corporate", "personal"}
        ):
    ...
    async def test_update_uses_authenticated_subject_as_actor(self) -> None:
        ...
        ) as update_memory, patch(f"{_POLICY}.PUBLIC_MODE", True), patch(f"{_POLICY}.is_privileged_user", return_value=False), patch(
            f"{_POLICY}.get_subject", return_value="user-1"
        ), patch(f"{_POLICY}._effective_domain_scope", return_value={"build", "corporate", "personal"}):
            await main.update(
                memory_id="mem-1",
    ...
    async def test_delete_requires_admin(self) -> None:
        with patch(f"{_POLICY}.PUBLIC_MODE", True), patch(f"{_POLICY}.is_privileged_user", return_value=False):
            with self.assertRaises(HTTPException) as ctx:
    ...
    async def test_delete_honors_admin_domain_scope(self) -> None:
        with patch(f"{_POLICY}.PUBLIC_MODE", True), patch(f"{_POLICY}.is_privileged_user", return_value=True), patch.object(
            main, "get_memory", new=AsyncMock(return_value=_memory_out(owner="user-1", tenant_id="tenant-a",))
        ), patch(f"{_POLICY}.get_domain_scope", return_value={"corporate"}):
            with self.assertRaises(HTTPException) as ctx:
    ...
    async def test_sync_check_hides_records_outside_admin_domain_scope(self) -> None:
        ...
        ), patch.object(main, "get_memory", new=AsyncMock(return_value=_memory_out())), patch(
            f"{_POLICY}.get_domain_scope", return_value={"corporate"}
        ):
            with self.assertRaises(HTTPException) as ctx:
