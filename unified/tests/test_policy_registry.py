...
class PolicyRegistryTests(unittest.IsolatedAsyncioTestCase):
    async def test_read_policy_registry_requires_admin(self) -> None:
        with (
            ...
            patch("src.security.policy.is_privileged_user", return_value=False),
        ):
    ...
    async def test_update_policy_registry_round_trips_via_main(self) -> None:
        registry = PolicyRegistry(
            ...
            subjects={
                "admin@example.com": {
                    "admin_domains": ["corporate", "build", "personal"]
                }
            },
        )
        with (
            patch("src.auth.PUBLIC_EXPOSURE", True),
            patch("src.security.policy.is_privileged_user", return_value=True),
        ):
            saved = await update_policy_registry(
                registry=registry, _user={"sub": "admin"}
            )
        self.assertEqual(saved.tenants["tenant-a"].write_domains, ["build"])
        self.assertEqual(
            saved.subjects["admin@example.com"].admin_domains,
            ["corporate", "build", "personal"],
        )
    ...
    def test_auth_loads_registry_from_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            registry_path.write_text(
                json.dumps({"tenants": {"tenant-a": {"read_domains": ["build"]}}}),
                encoding="utf-8",
            )
        ...
        self.assertEqual(
            auth.get_registry_domain_scope("", "tenant-a", "read"), {"build"}
        )
