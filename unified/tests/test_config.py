...
class TestConfigValidation:
    ...
    def test_oidc_cache_default(self, monkeypatch):
        """Test default OIDC discovery cache duration is 10 minutes."""
        from src import config
    ...
    def test_mcp_streamable_http_path_from_env(self, monkeypatch):
        ...
        monkeypatch.setenv("MCP_STREAMABLE_HTTP_PATH", "/custom/stream")
        config.get_config.cache_clear()

        cfg = config.get_config()
        assert cfg.mcp.streamable_http_path == "/custom/stream"

    def test_mcp_streamable_http_path_requires_leading_slash(self, monkeypatch):
        """Test streamable HTTP path must start with slash."""
        from src import config
    ...
    def test_mcp_streamable_http_path_normalizes_trailing_slash(self, monkeypatch):
        """Test streamable HTTP path has trailing slash removed."""
        from src import config
    ...
    def test_mcp_streamable_http_path_rejects_root_path(self, monkeypatch):
        """Test streamable HTTP path cannot be just root '/'."""
        from src import config
    ...
    def test_mcp_streamable_http_path_rejects_query(self, monkeypatch):
        """Test streamable HTTP path cannot contain query parameters."""
        from src import config

        monkeypatch.setenv("MCP_STREAMABLE_HTTP_PATH", "/stream?token=123")
        config.get_config.cache_clear()
    ...
    def test_mcp_streamable_http_path_rejects_fragment(self, monkeypatch):
        """Test streamable HTTP path cannot contain fragments."""
        from src import config

        monkeypatch.setenv("MCP_STREAMABLE_HTTP_PATH", "/stream#top")
        config.get_config.cache_clear()
    ...
    def test_mcp_streamable_http_path_rejects_spaces(self, monkeypatch):
        """Test streamable HTTP path cannot contain spaces."""
        from src import config

        monkeypatch.setenv("MCP_STREAMABLE_HTTP_PATH", "/my stream")
        config.get_config.cache_clear()
    ...
    def test_mcp_streamable_http_path_rejects_backslashes(self, monkeypatch):
        """Test streamable HTTP path cannot contain backslashes."""
        from src import config
    ...
    def test_mcp_streamable_http_path_rejects_double_slashes(self, monkeypatch):
        """Test streamable HTTP path cannot contain double slashes."""
        from src import config
    ...
    def test_mcp_streamable_http_path_has_length_limit(self, monkeypatch):
        """Test streamable HTTP path length cannot exceed limit."""
        from src import config

        monkeypatch.setenv("MCP_STREAMABLE_HTTP_PATH", "/" + "a" * 128)
        config.get_config.cache_clear()
    ...
    def test_mcp_streamable_http_path_rejects_dot_segment(self, monkeypatch):
        """Test streamable HTTP path cannot contain '.' segment."""
        from src import config

        monkeypatch.setenv("MCP_STREAMABLE_HTTP_PATH", "/my/./stream")
        config.get_config.cache_clear()

        with pytest.raises(ValueError, match="'.' or '..' segments"):
            config.get_config()
...
class TestConfigHelpers:
    ...
    def test_get_database_url(self, monkeypatch):
        ...
        config.get_config.cache_clear()
        assert config.get_database_url() == "postgresql+asyncpg://user:pass@host:5432/db"

    def test_is_public_mode(self, monkeypatch):
        """Test is_public_mode helper."""
        from src import config

        monkeypatch.setenv("PUBLIC_MODE", "true")
        monkeypatch.setenv("INTERNAL_API_KEY", "c" * 32)
        monkeypatch.setenv("OIDC_ISSUER_URL", "https://auth.example.com")
        config.get_config.cache_clear()
        assert config.is_public_mode() is True

    def test_get_public_base_url(self, monkeypatch):
        """Test get_public_base_url helper."""
        from src import config

        monkeypatch.setenv("PUBLIC_BASE_URL", "https://api.example.com")
        monkeypatch.setenv("INTERNAL_API_KEY", "d" * 32)
        monkeypatch.setenv("OIDC_ISSUER_URL", "https://auth.example.com")
        config.get_config.cache_clear()
        assert config.get_public_base_url() == "https://api.example.com"

    def test_get_oidc_issuer_url(self, monkeypatch):
        """Test get_oidc_issuer_url helper."""
        from src import config

        monkeypatch.setenv("OIDC_ISSUER_URL", "https://auth.example.com")
        config.get_config.cache_clear()
        assert config.get_oidc_issuer_url() == "https://auth.example.com"
