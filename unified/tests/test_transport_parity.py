...
class TransportParityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_parity_between_stdio_and_http(self) -> None:
        with patch("_gateway_src.main._client", return_value=_GatewayClient()), patch.object(
            ...
            gateway_result = await gateway.brain_search(query="payload", top_k=1, domain="build")
            transport_result = await mcp_transport.brain_search(query="payload", top_k=1, domain="build")
        self.assertEqual(transport_result, gateway_result)
