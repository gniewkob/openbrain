...
```
See [Dev Session Guide](./dev-session.md) for the full breakdown of prod vs dev tooling.
...
- **Obsidian HTTP endpoints returning `503 "Obsidian CLI command not found"`**: The `unified-server` container does not ship the Obsidian CLI binary; vault listing (`/api/v1/obsidian/vaults`) and bidirectional-sync status (`/api/v1/obsidian/sync-status`) work because they read filesystem mounts and Redis state respectively, but `read-note` / `write-note` / `update-note` / `sync` / `export` / `collection` / `bidirectional-sync` all need the CLI. **Production path:** call these via the stdio MCP gateway (running on the host where Obsidian and its CLI are installed) — `mcp__brain__brain_obsidian_*` tools — not directly over HTTP from inside the container. The HTTP routes exist for clients that already provide CLI access via `OBSIDIAN_CLI_COMMAND`.
- **`hidden_test_data_active_total` reads 0 even though records were marked with `custom_fields.test_data=true`**: Fixed in commit (see roadmap entry). The detector now recognizes both `metadata.test_data` (legacy/internal writes) and `metadata.custom_fields.test_data` (canonical placement when callers go through the V1 API write payload).
