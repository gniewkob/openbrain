...
7. **Backend URL Validation**: MCP backend URL (`BRAIN_URL`) must be a valid `http(s)` URL and must not include query params or fragment, to avoid silent runtime misrouting.
