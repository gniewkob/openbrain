...
7. **Backend URL Validation**: MCP backend URL (`BRAIN_URL`) must be a valid `http(s)` URL and must not include credentials/path/query/fragment, to avoid silent runtime misrouting. A trailing `/` is normalized away.
