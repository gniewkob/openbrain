#!/usr/bin/env python3
"""Guardrail: keep brain_search filter wiring aligned across transports."""
...
def _check_search_filter_parity(transport_src: str, gateway_src: str) -> list[str]:
    errors: list[str] = []
    transport_keywords = _extract_build_list_filter_keywords(
        transport_src, "brain_search"
    )
    gateway_keywords = _extract_build_list_filter_keywords(gateway_src, "brain_search")
    if transport_keywords != gateway_keywords:
        errors.append(
            "brain_search build_list_filters keyword drift: "
            f"transport={transport_keywords} gateway={gateway_keywords}"
        )

    required = {"domain", "entity_type", "owner", "sensitivity", "include_test_data"}
...
def main() -> int:
    ...
    errors = _check_search_filter_parity(transport_src, gateway_src)
    if errors:
        for error in errors:
            _fail(error)
        return 1
    print("Search filter parity guardrail passed.")
    return 0
